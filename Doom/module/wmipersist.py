...
if __name__ == '__main__':
    ...
    install_parser.add_argument('-filter', action='store', required=False,
                                help='the WQL filter string that will trigger'
                                     ' the script')
    ...
    if options.action.upper() == 'INSTALL':
        if (options.filter is None and options.timer is None) or (
                options.filter is not None and options.timer is not None):
