...
class Network(object):
    def __init__(self, *args, **kwargs):
        self.commonPort = [1, 3, 4, 6, 7, 9, 13, 17, 19, 20, 21, 22, 23, 24, 25, 26, 30, 32, 33, 37, 42, 43, 49, 53, 70,
                           ...
                           912, 981, 987, 990, 992, 993, 995, 999, 1000, 1001, 1002, 1007, 1009, 1010, 1011, 1021, 1022,
                           ...
                           9091, 9099, 9100, 9101, 9102, 9103, 9110, 9111, 9200, 9207, 9220, 9290, 9415, 9418, 9485,
                           9500, 9502, 9503, 9535, 9575, 9593, 9594, 9595, 9618, 9666, 9876, 9877, 9878, 9898, 9900,
    ...
    def generateIP(self, network, subnet):
        '''
    ...
    def pingsweepNetwork(self, network, subnet='24'):
        '''
            Ping Sweep scan. Scan for active hosts in network.
        '''
        try:
            ips = self.generateIP(network, subnet)
            p = multiprocessing.Pool(len(ips))
            activeIP = list(filter(self.RES, p.map(self.pingsweep, ips)))
    ...
    def singlePortScan(self, ip, port, openPort):
        '''
    ...
    def multiPortScan(self, ip, ports):
        '''
    ...
    def portScan(self, ip, ports):
        '''
            Port Scanning Main Method
        '''
        try:
            if type(ports) == int:
                return self.singlePortScan(ip, ports)
            elif type(ports) == list:
                res = self.multiPortScan(ip, ports)
                return res
        except KeyboardInterrupt:
            exit(0)
        except:
    ...
    def nmapScan(self, ip, ports):
        '''
            To Scan Service Version and Vulnerabality 
        '''
        portsStr = ','.join(ports)
        output = subprocess.Popen(["sudo", "nmap", "-sS", "-A", "-sV", "-p", portsStr, ip],
